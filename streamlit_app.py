...
def show(r):
    ...
    토지예측 = r["토지예측가_만원"]
    beta = B.get("beta", 500.0)
    rate = residual_rate(age, struct_key, B)
    바 = barea if barea > 0 else None

    st.metric((r["지번"] or r["PNU"]) + " · 토지 대지평당가 (사용연수와 무관)",
              f"{r['토지_대지평당가_만원']:,.0f} 만원/평",
              help="토지는 감가하지 않으므로 사용연수를 바꿔도 토지 평당가는 그대로입니다.")

    g1, g2, g3 = st.columns(3)
    g1.metric("▣ 토지 매입 적정가 (부지)", won(토지예측),
              help="재건축 시 기존 건물은 멸실 → 토지값이 매입 기준")
    if 바 is not None and 토지예측 is not None:
        신축건물 = beta * 바
        매도가 = 토지예측 + 신축건물
        잔존건물 = beta * rate * 바
        현시세 = 토지예측 + 잔존건물
        g2.metric(f"▣ 신축 매도 예상가 (0년·{바:,.0f}㎡)", won(round(매도가)),
                  help=f"토지 + 신축건물 {won(round(신축건물))} — 내가 지어 팔 때")
        g3.metric(f"(참고) 현 시세 (사용 {age}년·잔가율 {rate*100:.0f}%)", won(round(현시세)),
                  help=f"토지 + 잔존건물 {won(round(잔존건물))} — 기존 노후물건 현재가")
        st.caption(f"매입(토지) {won(토지예측)} → 신축 매도 {won(round(매도가))} · 차이 {won(round(신축건물))}"
                   f" (= 신축건물값, 사업 마진의 토대)")
    else:
        g2.metric("신축 매도 예상가", "연면적 입력 필요")
        g3.metric("현 시세", "연면적 입력 필요")
        st.caption("위 '건물 연면적(=대지면적×용적률)'을 입력하면 신축 매도가·현 시세가 계산됩니다.")
